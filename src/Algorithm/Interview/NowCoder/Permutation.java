...
public class Permutation {
    public ArrayList<String> Permutation(String str) {
        ...
        helper(chars, result, record, current);
        return result;
    }

    private void helper(char[] chars, ArrayList<String> result, int[] record, StringBuilder current){
        //todo: 长度匹配说明生成了一个新的排列
        if (current.length() == chars.length) {
            if (!result.contains(current.toString()))
                result.add(current.toString()); //todo: 有重复字符可能生成重复排列
            return;
        }
        //todo: 每次都循环查找所有元素
        for (int i = 0; i < chars.length; i++){
            ...
            if (record[i] == 0){
                ...
                //todo: 回溯
                record[i] = 1;
                helper(chars, result, record, temp);
                record[i] = 0;
            }
        }
    }
